...
public class ControllerAddRemoveRoles {
	...
	protected static void repaintTheWindow() {
		...
		ViewAddRemoveRoles.getTheStage().setTitle("CSE 360 Foundation Code: Admin Opertaions Page");
		ViewAddRemoveRoles.getTheStage().setScene(ViewAddRemoveRoles.theAddRemoveRolesScene);
		ViewAddRemoveRoles.getTheStage().show();
	}
	...
	protected static void performRemoveRole() {
		...
		if (ViewAddRemoveRoles.theRemoveRole.compareTo("<Select a role>") != 0) {
			...
			// If an actual role was selected, update the database entry for that user for the role
			// But first, check if the current role chosen is an admin.
			//if the chosen role is not admin, proceed to the removing process.
			if (ViewAddRemoveRoles.theRemoveRole.compareTo("Admin")==0) {
				ViewAddRemoveRoles.alertRemoveAdminRole.setTitle("**** WARNING ****");
				ViewAddRemoveRoles.alertRemoveAdminRole.setHeaderText("Role removal issue");
				ViewAddRemoveRoles.alertRemoveAdminRole.setContentText("Admin cannot remove another admin's role.");
				ViewAddRemoveRoles.alertRemoveAdminRole.showAndWait();
			}
			else {
				if (ViewAddRemoveRoles.removeList.size()<=2) {	//including first item <Select a Role>
					ViewAddRemoveRoles.alertRemoveOnlyRole.setTitle("**** WARNING ****");
					ViewAddRemoveRoles.alertRemoveOnlyRole.setHeaderText("Role removal issue");
					ViewAddRemoveRoles.alertRemoveOnlyRole.setContentText("Each user need to play at least 1 role.");
					ViewAddRemoveRoles.alertRemoveOnlyRole.showAndWait();
				}
				else 
				{ 
					if(theDatabase.updateUserRole(ViewAddRemoveRoles.theSelectedUser, 
	...
	protected static void performReturn() {
		guiAdminHome.ViewAdminHome.displayAdminHome(ViewAddRemoveRoles.getTheStage(),
				ViewAddRemoveRoles.getTheUser());
	}
	...
	protected static void performLogout() {
		guiUserLogin.ViewUserLogin.displayUserLogin(ViewAddRemoveRoles.getTheStage());
	}
