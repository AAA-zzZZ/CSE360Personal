...
public class ViewNewAccount {
	...
	*/
	...
	static protected Label noUsernameInputFound = new Label();
	...
	static protected Label noPasswordInputFound = new Label();	
	...
	*/
	...
	public static void displayNewAccount(Stage ps, String ic) {
		...
		text_Password1.textProperty().addListener((observable, oldValue, newValue) 
				-> {ControllerNewAccount.updateNewAccountPassword(); });
		
		// Establish an error message for when there is no input
		setupLabelWidget(noPasswordInputFound, 50, 300, "Arial", 14, width-10, Pos.BASELINE_LEFT);	
		noPasswordInputFound.setText("No password input text found!");	// This changes based on input, but we 
		noPasswordInputFound.setTextFill(Color.RED);			// want this to be shown at startup	
    	...
    	theRootPane.getChildren().addAll(label_NewUserCreation, label_NewUserLine, text_Username,
    			label_UsernameRequirements, noUsernameInputFound, validUsername,
    			text_Password1, text_Password2, 
    			label_PasswordRequirements, noPasswordInputFound, validPassword, 
	...
	private ViewNewAccount() {
		...
		setupTextUI(text_Username, "Arial", 18, 300, Pos.BASELINE_LEFT, 50, 160, 
				true);
		...
		setupTextUI(text_Password2, "Arial", 18, 300, Pos.BASELINE_LEFT, 50, 260, 
				true);
		text_Password2.setPromptText("Enter Password Again");
        ...
        button_UserSetup.setStyle("-fx-font: 22 arial; -fx-base: #8C1D40;");
